...
import { createRelationship, getRelationship, updateRelationship } from '@/lib/api/relationships'
...
export default function RelationshipFormPage({ relationshipId = null }) {
  ...
  useEffect(() => {
    ...
    getRelationship(relationshipId)
      .then((data) => setForm({ ...data, start_date: data.start_date || '' }))
      .catch((err) => setError(errorMessage(err)))
      .finally(() => setLoading(false))
  }, [editing, relationshipId])
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      router.push(`/relationships/${saved.relationship_id}`)
      router.refresh()
    } catch (err) {
  ...
  return <div className="page narrow">
    <header className="page-header"><div><p className="eyebrow">{editing ? 'EDIT' : 'NEW RELATIONSHIP'}</p><h2>{editing ? '관계 정보 수정' : '새 관계 등록'}</h2></div><Link className="secondary-button" href={editing ? `/relationships/${relationshipId}` : '/relationships'}>취소</Link></header>
