...
import { getDashboardSummary } from '../api/relationshipStats'
...
export default function DashboardPage() {
  ...
  useEffect(() => { getDashboardSummary().then(setData).catch((e) => setError(errorMessage(e))) }, [])
  ...
  return (
    <div className="page">
      <header className="page-header">
        ...
        <Link className="primary-button" to="/relationships/new">관계 등록</Link>
      </header>
      ...
      {data && <>
        ...
        <section className="two-column">
          <article className="panel">
            <div className="panel-heading"><h3>최근 기록</h3><Link to="/relationships">전체 관계 보기</Link></div>
            {data.recent_logs.length === 0 ? <Empty>첫 관계와 기록을 추가해보세요.</Empty> : <div className="list-stack">
              {data.recent_logs.map((log) => <Link className="list-row" key={log.log_id} to={`/logs/${log.log_id}`}>
          ...
          <div className="panel-stack">
            <article className="panel">
              ...
              {data.low_temperature_relationships.length === 0 ? <Empty>현재 주의가 필요한 관계가 없습니다.</Empty> : data.low_temperature_relationships.map((item) => <Link className="temperature-row" key={item.relationship_id} to={`/relationships/${item.relationship_id}`}><span>{item.name}</span><strong>{item.temperature_score}°</strong></Link>)}
            </article>
            <article className="panel">
              <h3>최근 부정 기록</h3>
              {data.relationships_with_negative_logs.length === 0 ? <p className="muted">최근 30일간 부정 기록이 없습니다.</p> : data.relationships_with_negative_logs.map((item) => <Link className="temperature-row" key={item.relationship_id} to={`/relationships/${item.relationship_id}`}><span>{item.name}</span><strong>{item.negative_log_count}건</strong></Link>)}
            </article>
          </div>
        </section>
      </>}
