...
import { getEmotionTags } from '../api/emotionTags'
import { createLog, getLog, updateLog } from '../api/relationshipLogs'
import { ErrorNotice, Loading } from '../components/PageState'
import { eventTypes } from '../constants'

const initial = { log_date: new Date().toISOString().slice(0, 10), title: '', content: '', event_type: 'conversation', emotion_score: 0, importance_score: 3, my_action: '', other_action: '', result: '', memo: '', emotion_tag_ids: [] }

export default function LogFormPage() {
  const { relationshipId, logId } = useParams()
  ...
  useEffect(() => { Promise.all([getEmotionTags(), editing ? getLog(logId) : Promise.resolve(null)]).then(([tagData, log]) => { setTags(tagData); if (log) setForm({ ...log, emotion_tag_ids: log.emotion_tags.map((tag) => tag.emotion_tag_id) }) }).catch((e) => setError(errorMessage(e))).finally(() => setLoading(false)) }, [editing, logId])
  ...
  const submit = async (e) => { e.preventDefault(); setError(''); try { const payload = { ...form, emotion_score: Number(form.emotion_score), importance_score: Number(form.importance_score) }; const saved = editing ? await updateLog(logId, payload) : await createLog(relationshipId, payload); navigate(`/logs/${saved.log_id}`) } catch (err) { setError(errorMessage(err)) } }
  const back = editing ? `/logs/${logId}` : `/relationships/${relationshipId}`
  return <div className="page narrow"><header className="page-header"><div><p className="eyebrow">JOURNAL</p><h2>{editing ? '기록 수정' : '새 기록 작성'}</h2><p>사실과 감정을 구분해 적으면 흐름을 더 잘 볼 수 있습니다.</p></div><Link className="secondary-button" to={back}>취소</Link></header><section className="panel form-panel"><ErrorNotice message={error} /><form className="form-grid" onSubmit={submit}>
